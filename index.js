...
// FUNÇÃO DE HIGIENIZAÇÃO (BLINDAGEM TOTAL CONTRA ERRO 400 DE ALTERNÂNCIA)
...
app.post('/api/perguntar', async (req, res) => {
  ...
  // Limpa o histórico antes de enviar para os modelos rígidos da NVIDIA
  ...
  try {
    ...
    // 👁️ FLUXO 2: PRÉ-ANÁLISE DE VISÃO (Dá "olhos" a todos os modelos cegos do ringue e ao Juiz)
    if (arquivoAnexo && arquivoAnexo.tipo === 'imagem') {
      console.log(`[Cactus-Vision] Executando transcrição visual analítica...`);
      try {
          ...
          messages: [
            {
              role: "user",
              content: [
                { type: "text", text: "Transcreva textualmente e descreva detalhadamente todos os diagramas, fluxogramas, setas, tabelas ou textos científicos presentes nesta imagem para que um modelo textual cego compreenda perfeitamente o assunto." },
        ...
        console.log(`[Cactus-Vision] Mapeamento de imagem concluído.`);
      } catch (errVision) {
        console.error("Falha na API de Visão:", errVision);
        decolagemVisaoTexto = "Erro ao extrair dados visuais do arquivo.";
      }
    }

    // 🧠 CONFIGURAÇÃO DA DIRETRIZ MASTER UNIFICADA
    ...
    // Alimenta o cérebro de todos os modelos com os dados reais extraídos da foto!
    if (decolagemVisaoTexto) {
      textoInstrucaoSistema += `\n\n[IMAGEM ANEXADA (CONTEÚDO TEXTUAL/VISUAL DETECTADO NO ARQUIVO)]:\n${decolagemVisaoTexto}`;
    }
    ...
    // DISPARO EM PARALELO DO RINGUE (Todos respondendo com base no infográfico mapeado)
    ...
    // O Juiz agora lê as opções sabendo exatamente do que se trata a imagem
    const promptJuiz = `
